...
> Last updated: 2026-04-30 | Session: 5 EAS builds → TestFlight verified on iPhone with NOVA icon + animated logo + maps
...
| 2026-04-29 | Ride tip flow + chauffeur UX & marker fixes | (1) **Ride tip on completion**: `HomeScreen.tsx` ride-completed card now shows Fare / Tip / Total summary + tip chips (No tip / 10% / 15% / 20% / Custom) + custom € input. Mirrors bersenev's HomeScreen tip UI but uses VIP's cream-gold (`#d9c0a4`) chip-active styling. CONFIRM resets tip + rating state. Tip pricing base = `routeInfo.distance × 2.5 × rideTypeMult` (sedan 1.0 / suv 1.3 / van 1.6). (2) **Chauffeur entry-point UX**: removed in-screen "Resume active chauffeur trip →" banner from `ChauffeurSearchScreen` — confusing on first open. ChauffeurSearch now opens directly in `'search'` mode (idle state + its banner removed entirely). Resume routing moved to entry point: `HomeScreen` chauffeur button reads `useChauffeurStore.getState().trip` and navigates to `ChauffeurActive` if active, else to `ChauffeurSearch`. (3) **Navigation fixes**: changed handleRequest from `navigate` → `replace('ChauffeurActive')` so ChauffeurSearch unmounts cleanly. Removed `presentation: 'transparentModal'` from ChauffeurSearch in `MainNavigator.tsx` — was leaving the search screen visible underneath ChauffeurActive after request. Removed `useEffect(() => { if (!trip) navigation.goBack() }, [trip, navigation])` bounce-back in `ChauffeurActiveScreen.tsx` — was firing on mount in edge cases. (4) **Map marker flicker fix**: stop pins were `COLORS.primary` (`#1a1a2e`) — invisible on dark map + flickered because 1-Hz `tick()` re-rendered markers each second. Switched to `#d9c0a4` (active) / `#888888` (completed) and added `tracksViewChanges={false}` to all 3 chauffeur markers (pickup, stops, driver). Pickup pin is now white with black border for contrast. (5) **iPhone 17 (plain) simulator booted** alongside iPhone 17 Pro for dual-window dev — both connect to `exp://127.0.0.1:8081`. Expo Go on App Store now SDK 54 only — physical-device path is blocked until EAS dev build (still awaiting Apple credentials). |
| 2026-04-29 | Mobile app rebrand VIP Mobility → NOVA DRIVE | User received Apple Developer credentials and chose full rebrand (option b). Apple ID `aleksandrsvch@icloud.com`, Team ID `TMU9V5MBJ9`. Initially picked bundle `com.nova.app` then `com.novaride.app` — both globally taken on Apple's registry. Settled on **`com.aleksandrsvch.nova`** (personal namespace). Picked App Store display name **NOVA DRIVE** (originally "NOVA"). App Store Connect record created → App ID `6764670213` (https://appstoreconnect.apple.com/apps/6764670213). Bundle ID registered at developer.apple.com with **Push Notifications** capability only (Apple Maps not needed since app uses Google Maps). Final code state: (1) `app.json`: name `NOVA DRIVE`, slug `nova-drive`, ios.bundleIdentifier + android.package `com.aleksandrsvch.nova`, all 5 NS*UsageDescription strings reference NOVA DRIVE, expo-location plugin string updated, Stripe `merchantIdentifier: merchant.com.aleksandrsvch.nova`. (2) `package.json` name `nova-mobile`. (3) `eas.json`: production env URLs to `https://api.novadrive.app/v1` + `wss://api.novadrive.app` (placeholder — no real domain yet); `submit.production.ios` fully populated with all 3 fields (appleId, appleTeamId, ascAppId 6764670213). (4) UI text rebranded in 7 spots to "NOVA DRIVE": `LoginScreen` title, `RegisterScreen` subtitle, `KYCScreen` justification, `ProfileScreen` + `OwnerProfileScreen` version line, `SupportScreen` greeting, `navigation/index.tsx` loading spinner. (5) `OwnerProfileScreen` support email → `support@novadrive.app`, phone "+971 800 NOVA". (6) `mobile/TOMORROW.md` rewritten as final pre-build checklist with all credentials inline. (7) Backend NOT rebranded — PM2 app name, Postgres DB, deploy path all still `vip-mobility` (server-side migration deferred; mobile talks to backend over HTTP regardless of naming). (8) Project directory `/Users/alex/project_11/project_1/` unchanged. **Slug change `vip-mobility` → `nova-drive`** uses same Expo projectId UUID (`7d58a451…`) — first build will register new slug on server. **Lesson**: short popular `com.nova.*` patterns are heavily squatted on Apple's registry — fall straight to a personal namespace bundle ID for unique apps. **Build-ready** — next session run `eas credentials` → `eas build --platform ios --profile production`. **Still needed before App Store submission**: real 1024×1024 NOVA DRIVE icon (current is 70-byte placeholder), production API domain + TLS cert, Privacy Policy URL, optional Apple Pay merchant ID `merchant.com.aleksandrsvch.nova`. |
| 2026-04-30 | 5 EAS builds → TestFlight live on iPhone with NOVA branding | Single long session through 5 production builds reaching working app on physical iPhone. **Auth**: ran `eas credentials` interactively, generated Distribution Cert (serial `1CF330E8B7D328A6F81B141123847897`, exp 2027-04-29) + Provisioning Profile (`945AU4NQA7`). App-specific password `qazg-dgti-rdyk-clft` for non-interactive `eas build/submit` (passed via `EXPO_APPLE_APP_SPECIFIC_PASSWORD` env). Apple Pay capability + merchant ID `merchant.com.aleksandrsvch.nova` auto-created by EAS during credential setup. **Build #1** failed with "slug `nova-drive` doesn't match Expo project slug `vip-mobility`" — reverted slug locally to `vip-mobility` (slug is internal, not user-visible). **Build #2-3** rejected by Apple "Starting April 28 2026, Apple requires Xcode 26+" — fixed by adding `image: "latest"` to all iOS profiles in `eas.json`. **Build #3** uploaded successfully, TestFlight worked, but login failed with network error → ATS issue. Added `NSAppTransportSecurity.NSExceptionDomains["109.120.133.113"]` + `NSExceptionAllowsInsecureHTTPLoads: true` in app.json infoPlist. Also added `ITSAppUsesNonExemptEncryption: false` to skip manual export-compliance prompt. **Build #4** login worked but Maps tiles wouldn't render — Google Maps API key was OK (Application restrictions = None) but Maps SDK for iOS needed enabling at TWO places: tick on key's API restrictions list AND enable in project library at https://console.cloud.google.com/apis/library/maps-ios-backend.googleapis.com. Saved this gotcha as `reference-google-maps-key.md`. **Build #5** rebrand: app.json `name: "NOVA"` (was "NOVA DRIVE") + all in-app text, plus new icon set (assets/icon.png, adaptive-icon.png, splash.png, notification-icon.png all generated from user-supplied `assets/novaicon.png` 1336×1336 via `sips --resampleHeightWidth`) and animated logo on LoginScreen. **Note: App Store listing name remains "NOVA DRIVE"** (Apple enforces uniqueness across the App Store, "NOVA" was taken). Home-screen `CFBundleDisplayName` and in-app text say "NOVA". **GIF animation gotcha**: tried expo-image v1.10.6 with `assets/novalogo.gif` (150 frames, 33fps, 4.5s loop) — rendered first frame only on Expo Go SDK 50, no animation despite `autoplay`. Switched to `expo-av` v13.10.6 with `ffmpeg`-converted `assets/novalogo.mp4`, `<Video shouldPlay isLooping isMuted>` — works in both Expo Go and production builds. Saved as `feedback-expo-go-gif-animation.md`. **TOMORROW.md updated**. **Open work before App Store launch (TestFlight is fine)**: real production domain + Let's Encrypt cert (Apple Review will reject ATS exception), Privacy Policy URL, screenshots under NOVA branding, App Store description/keywords, Google Maps key bundle restriction (currently unrestricted = abuse risk + cost), Google Play submit JSON. |
